...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!fixtureId) {
    document.getElementById("matchDetails").innerHTML = "<h2>Match not found</h2>"
  ...
  const loadFallbackData = () => {
    isOfflineMode = true
    updateStatus("⚠️ Using offline data. Some information may not be current.", true)
    ...
    return {
      fixture: {
        ...
        teams: {
          home: { name: "Home Team", logo: "/placeholder.svg?height=100&width=100", id: 40 },
          away: { name: "Away Team", logo: "/placeholder.svg?height=100&width=100", id: 41 },
        },
        fixture: {
      ...
      stats: [
        {
          team: { name: "Home Team", logo: "/placeholder.svg?height=100&width=100" },
          statistics: [
        ...
        {
          team: { name: "Away Team", logo: "/placeholder.svg?height=100&width=100" },
          statistics: [
  ...
  async function fetchFixtureDetails(fixtureId) {
    ...
    try {
      const fixtureRes = await fetch(`https://v3.football.api-sports.io/fixtures?id=${fixtureId}`, {
        method: "GET",
      ...
      document.getElementById("matchTitle").textContent = `${homeTeam.name} vs ${awayTeam.name}`
      ...
      const cachedData = loadCachedFixtureDetails()
      if (cachedData) {
        updateStatus("⚠️ Using cached match data. Some information may not be current.", true)
        return cachedData
      }
  ...
  async function initializeMatchDetails() {
    ...
    if (!fixtureDetails || !fixtureDetails.fixture) {
      document.getElementById("matchDetails").innerHTML = "<h2>Failed to load match details</h2>"
      updateStatus("Failed to load match details. Please try again later.", true)
      return
    }
  ...
  function renderFixtureDetails(fixtureDetails) {
    ...
    document.getElementById("matchTitle").textContent = `${fixture.teams.home.name} vs ${fixture.teams.away.name}`
    ...
    if (!isFutureMatch) {
      ...
      renderMatchStatistics(fixtureDetails.stats, fixture.teams.home.name, fixture.teams.away.name)
    } else {
      ...
      if (statsContainer) {
        statsContainer.innerHTML = `
          ...
          <div class="preview-message">
            ...
            <p>Match scheduled for: ${new Date(fixture.fixture.date).toLocaleString()}</p>
          </div>
        `
      }
    }
    ...
    const hasLineupData = fixtureDetails.lineups && fixtureDetails.lineups.length >= 2
    ...
    if (lineupsContainer) {
      if (hasLineupData) {
        ...
        renderLineups(fixtureDetails.lineups, fixture.teams.home.id, fixture.teams.away.id)
      } else {
    ...
    renderStandings(fixtureDetails.standings, fixture.teams.home.id, fixture.teams.away.id, fixture.league)
      ...
      <div class="match-header-enhanced">
        <div class="match-competition">
          <span class="competition-logo">
            <img src="${league.logo || "/placeholder.svg?height=30&width=30"}" alt="${league.name}" class="competition-img">
          </span>
          <span class="competition-name">${league.name}, Round ${league.round?.replace("Regular Season - ", "") || ""}</span>
        </div>
        ...
        <div class="match-teams-container">
          <div class="match-team home-team">
            <div class="team-logo-container">
              <img src="${homeTeam.logo}" alt="${homeTeam.name}" class="team-logo-large">
            </div>
          ...
          <div class="match-team away-team">
            <div class="team-logo-container">
              <img src="${awayTeam.logo}" alt="${awayTeam.name}" class="team-logo-large">
            </div>
  ...
  function renderStandings(standingsData, homeTeamId, awayTeamId, league) {
    ...
    if (!standingsContainer) {
      ...
      if (lineupsContainer && lineupsContainer.nextSibling) {
        lineupsContainer.parentNode.insertBefore(standingsContainer, lineupsContainer.nextSibling)
      } else if (lineupsContainer) {
        lineupsContainer.parentNode.appendChild(standingsContainer)
      } else {
        document
          .getElementById("matchDetails")
          .parentNode.insertBefore(standingsContainer, document.getElementById("matchDetails"))
      }
    }
    ...
    try {
      ...
      const homeTeamStanding = standings.find((team) => team.team.id === homeTeamId)
      const awayTeamStanding = standings.find((team) => team.team.id === awayTeamId)
      ...
      document.getElementById("viewFullStandings").addEventListener("click", () => {
  ...
  function showFullStandings(standings, homeTeamId, awayTeamId) {
    ...
    standings.forEach((team) => {
      ...
      const rowClass = isHomeTeam ? "home-team-standing" : isAwayTeam ? "away-team-standing" : ""
      ...
      modalHTML += `
        <tr class="team-standing ${rowClass}">
          ...
          <td class="team-column">
            <div class="team-info">
              <img src="${team.team.logo}" alt="${team.team.name}" class="standing-team-logo">
    ...
    modalContainer.querySelector(".close-standings-modal").addEventListener("click", () => {
  ...
  function renderLineups(lineups, homeTeamId, awayTeamId) {
    ...
    if (homeFormationEl) homeFormationEl.textContent = homeLineup.formation || "N/A"
    if (awayFormationEl) awayFormationEl.textContent = awayLineup.formation || "N/A"

    // Update coach names
    const homeCoachNameEl = document.getElementById("homeCoachName")
    const awayCoachNameEl = document.getElementById("awayCoachName")

    if (homeCoachNameEl) homeCoachNameEl.textContent = homeLineup.coach?.name || "Unknown"
    if (awayCoachNameEl) awayCoachNameEl.textContent = awayLineup.coach?.name || "Unknown"
    ...
    if (homeCoachImg) homeCoachImg.src = homeLineup.coach?.photo || homeLineup.team.logo
    if (awayCoachImg) awayCoachImg.src = awayLineup.coach?.photo || awayLineup.team.logo
    ...
    if (homeSubsContainer) {
      ...
      renderSubstitutes(homeSubsContainer, homeLineup.substitutes || [], "home")
    }

    if (awaySubsContainer) {
      awaySubsContainer.innerHTML = "" // Clear existing substitutes
      renderSubstitutes(awaySubsContainer, awayLineup.substitutes || [], "away")
    }
  }
  ...
  function renderMatchStatistics(stats, homeName, awayName) {
    if (!stats || stats.length < 2) {
      ...
      const placeholderStats = [
        {
          team: { name: homeName, logo: "/placeholder.svg?height=100&width=100" },
          statistics: [
            { type: "Ball Possession", value: "57%" },
            { type: "Passes", value: "498" },
            { type: "Shots Total", value: "12" },
        ...
        {
          team: { name: awayName, logo: "/placeholder.svg?height=100&width=100" },
          statistics: [
            { type: "Ball Possession", value: "43%" },
            { type: "Passes", value: "375" },
            { type: "Shots Total", value: "11" },
    ...
    const homePossession = Number.parseInt(
      stats[0].statistics.find((stat) => stat.type === "Ball Possession")?.value.replace("%", "") || 50,
    )
    const awayPossession = Number.parseInt(
      stats[1].statistics.find((stat) => stat.type === "Ball Possession")?.value.replace("%", "") || 50,
    )

    // Create possession chart
    createPossessionChart(homePossession, awayPossession, homeTeam.name, awayTeam.name)
    ...
    topStatsTypes.forEach((statType) => {
      ...
      const findStat = (team, type) => {
        ...
        if (type === "Ball Possession %" && team.statistics.find((s) => s.type === "Ball Possession")) {
          return team.statistics.find((s) => s.type === "Ball Possession").value
        }
    ...
    const allStatTypes = [
      ...new Set([...stats[0].statistics.map((s) => s.type), ...stats[1].statistics.map((s) => s.type)]),
    ]

    allStatTypes.forEach((statType) => {
      const homeStat = stats[0].statistics.find((s) => s.type === statType)?.value || "0"
      const awayStat = stats[1].statistics.find((s) => s.type === statType)?.value || "0"
  ...
  function createPossessionChart(homePossession, awayPossession, homeName, awayName) {
  ...
  function renderHeadToHead(h2h) {
    ...
    h2h.forEach((match) => {
      ...
      else if (match.score.fulltime.home < match.score.fulltime.away) awayWins++
      else draws++
    })
  ...
  function setupStreamLinks(fixtureId) {
    ...
    if (links.length > 0) {
      ...
      messageContainer.textContent = "No live streams available for this match at the moment."
    }
  }
  ...
  document.querySelectorAll(".footer-toggle").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      document.getElementById("content-text").innerHTML = contentData[target] || "Content not available"
